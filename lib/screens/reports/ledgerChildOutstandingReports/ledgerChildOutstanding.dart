...
class LedgerChildOutstandingReportScreen extends StatefulWidget {
  @override
  _LedgerChildOutstandingReportScreenState createState() =>
      _LedgerChildOutstandingReportScreenState();
}

class _LedgerChildOutstandingReportScreenState
    extends State<LedgerChildOutstandingReportScreen> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        setState(() {
          ...
          // Use a Set to store unique ledgerIds
          Set<int> ledgerIds =
              Invoices.map((entry) => entry['ledgerId'] as int).toSet();

          uniqueLedgerIds = ledgerIds.toList();
          getSalesPerson();
          print('uniqueLedgerIds$uniqueLedgerIds');
  ...
  getSalesPerson() async {
    try {
      var requestBody = {
        "ledgers": uniqueLedgerIds,
        "sessionId": currentSessionId
      };

      var response = await multiledgerService(requestBody);
      var decodedData = jsonDecode(response.body);
      if (response.statusCode == 200) {
        setState(() {
          List<Map<String, dynamic>> ledgersInfo =
              List<Map<String, dynamic>>.from(decodedData);
          parentOutstandings = ledgersInfo;
          for (var ledger in ledgersInfo) {
            salesPerson = ledger['salesperson'];
          }
          selectedParent = parentOutstandings[0]['id'];
          filterLedgers(selectedParent);
          print('selectedParent$selectedParent');
  ...
  filterLedgers(ledgerId) {
    childOutstandings = Invoices.where((entry) {
      return entry['ledgerId'] == ledgerId;
    }).toList();
    totalRows = 00;
    double refAmount = 00;
    ...
    totalGrandAmnt = 00;
    totalPending = 00;
    for (var invoice in childOutstandings) {
      totalRows++;
          ...
          isLoading
              ...
              : Expanded(
                  child: ListView.builder(
                    itemCount: childOutstandings.length,
                    itemBuilder: (context, index) {
                      var invoice = childOutstandings[index];
