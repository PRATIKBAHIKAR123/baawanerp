    ...
    try {
      ...
      if (response.statusCode == 200) {
        setState(() {
          ...
          var reportWithHeaderAndFooter = printReports!.firstWhere(
            (report) => report['reportName'] == 'With Header And Footer',
            orElse: () => null,
          );

          // If found, set reportFileName, else fallback to isDefault report
          if (reportWithHeaderAndFooter != null) {
            reportFileName = reportWithHeaderAndFooter['fileName']
                .replaceFirst(RegExp(r'^\\+'), '');
          } else {
            var defaultReport = printReports!.firstWhere(
              (report) => report['isDefault'] == true,
              orElse: () => null,
            );
            if (defaultReport != null) {
              reportFileName =
                  defaultReport['fileName'].replaceFirst(RegExp(r'^\\+'), '');
            } else {
              // Handle case where no default report is found (optional)
              reportFileName = null;
            }
          }
        });
